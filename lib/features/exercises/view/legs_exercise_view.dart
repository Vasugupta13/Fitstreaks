...
class LegsExerciseView extends StatelessWidget {
  const LegsExerciseView({super.key});
      ...
      body: Obx(
          ...
          children: [
            ExerciseTopInfoCardWidget(
              time: exerciseController.legsDuration.value,
              description: allExercise[3]["description"],
              level: exerciseController.levelName.value == "Intermediate"
                  ? "Medium"
                  : exerciseController.levelName.value,
              cals: exerciseController.legsCalories.value,
            ),
            Padding(
            ...
            ListView.builder(
              shrinkWrap: true,
              itemCount: exerciseController.legsList.length,
              physics: const NeverScrollableScrollPhysics(),
              itemBuilder: (context, index) {
                return ExerciseListCommonWidget(
                  image: exerciseController.legsList[index].image,
                  name: exerciseController.legsList[index].name,
                  index: index,
                  list: exerciseController.legsList,
                );
              },
            ),
          ],
