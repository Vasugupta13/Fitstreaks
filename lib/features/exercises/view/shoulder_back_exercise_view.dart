...
class ShoulderBackExerciseView extends StatelessWidget {
  const ShoulderBackExerciseView({super.key});
      ...
      body: Obx(
          ...
          children: [
            ExerciseTopInfoCardWidget(
              time: exerciseController.sbDuration.value,
              description: allExercise[5]["description"],
              level: exerciseController.levelName.value == "Intermediate"
                  ? "Medium"
                  : exerciseController.levelName.value,
              cals: exerciseController.sbCalories.value,
            ),
            Padding(
            ...
            ListView.builder(
              shrinkWrap: true,
              itemCount: exerciseController.sbList.length,
              physics: const NeverScrollableScrollPhysics(),
              itemBuilder: (context, index) {
                return ExerciseListCommonWidget(
                  image: exerciseController.sbList[index].image,
                  name: exerciseController.sbList[index].name,
                  index: index,
                  list: exerciseController.sbList,
                );
              },
            ),
          ],
